...
try:
    ...
    if conn.is_connected():
        print("connection mysql sccussfully ")
except Error as e:
    print("can not connection mysql:", e)

# إنشاء مؤشر لتنفيذ العمليات
...
current_date = datetime.datetime.now().strftime("%Y_%m_%d")  # استبدال الشرطة بشرطة سفلية
...
class Face_Recognizer:
    ...
    def draw_note(self, img_rd):
        ...
        cv2.putText(img_rd, "FaceRecognizerDeepLearning", (20, 40), self.font, 1, (255, 255, 255), 1, cv2.LINE_AA)
    ...
    # التحقق مما إذا كان الاسم قد تم تسجيل الحضور به في التاريخ الحالي والوقت الحالي
        ...
        if existing_entry:
            ...
            conn.commit()
            print(f"{name} marked as detectd for {current_date} at {current_time}")
            api_url = "http://localhost:5000/signal"  # تغيير العنوان إلى عنوان الخادم الخاص بك
            response = requests.post(api_url, data={'signal': '0'})  # تغيير القيمة إلى '0'
            print("Signal sent:", response.status_code)
            
                               
            print('doeing  detsct and insert the face to Database')
            conn.close()
                ...
                if (self.current_frame_face_cnt == self.last_frame_face_cnt) and (
                    ...
                    if "unknown" in self.current_frame_face_name_list:
                        ...
                        if  count_sure>=15:
                            ...
                            if conn.is_connected():
                                ...
                                cursor.execute("INSERT INTO face_unknown (place, time, date,ipCamera,numCamera) VALUES (%s, %s, %s, %s, %s)", ("roof", "current_time", "current_date","10.10.2.1","1"))
                        # cursor.execute("INSERT INTO face_detect2 (name, time, date) VALUES (%s, %s, %s)", ("name", "current_time", "current_date"))
                                conn.commit()
                        # print(f" unknown marked as detectd for {current_date} at {current_time}")
                                 # إرسال الإشارة 0 عبر API
                                print('doeing detect and insert the Unknown faceSignal  to Database')
                                conn.close()
                    ...
                    if self.current_frame_face_cnt != 0:
                        ...
                        # Loop through the facial landmarks and draw them as points
                            ...
                            for i in range(shape.num_parts):
                                x = shape.part(i).x
                                y = shape.part(i).y
                                cv2.circle(img_rd, (x, y), 2, (0, 255, 0), -1)
                    ...
                    for i in range(self.current_frame_face_cnt):
                            ...
                            # Draw green rectangle with transparent background
                                ...
                                rect_h = 180  # Height of the rectangle
                            ...
                            cv2.LINE_AA)
                            # Write text on the green rectangle
                                cv2.putText(img_rd, "Age", (rect_x + rect_w + 10, rect_y +  35), cv2.FONT_HERSHEY_COMPLEX, 0.4, (51, 25, 0), 1, cv2.LINE_AA)
                                cv2.putText(img_rd, "Sana'a", (rect_x + rect_w + 10, rect_y +  56), cv2.FONT_HERSHEY_COMPLEX, 0.4, (51, 25, 0), 1, cv2.LINE_AA)
                                cv2.putText(img_rd, "job", (rect_x + rect_w + 10, rect_y + 86), cv2.FONT_HERSHEY_COMPLEX, 0.4, (51, 25, 0), 1, cv2.LINE_AA)
                                cv2.putText(img_rd, "gender", (rect_x + rect_w + 10, rect_y + 116), cv2.FONT_HERSHEY_COMPLEX, 0.4, (51, 25, 0), 1, cv2.LINE_AA)
                                cv2.putText(img_rd, "sig", (rect_x + rect_w + 10, rect_y + 125), cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 0.4, (51, 25, 0), 1, cv2.LINE_AA)
    ...
    def new_method(self, faces):
        return faces
                # print(f'#################################{self.face_name_known_list[similar_person_num]}')
